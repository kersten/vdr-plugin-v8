    ...
    if (dp != NULL) {
        while (ep = readdir(dp)) {
            ...
            if (ep->d_type == isFolder) {
                ...
                Plugin v8;
                v8.init(index_file, menu);
                
                Plugins.push_back(v8);
            } else {
                dsyslog("V8: Found plugin: %s", ep->d_name);

                char index_file[strlen(plugin_dir) + strlen(ep->d_name)];
...
bool cPluginV8::Start(void) {
    ...
    for(int i=0;i<Plugins.size();i++) {
        dsyslog("Started V8 plugin: %s", (char*)Plugins[i].getName());
        //Plugins[i].init(index_file, menu);
    }

    /*/for (itr = plugins.begin(); itr != plugins.end(); ++itr) {
        dsyslog("Starting V8 plugin: %s", *itr);
    }*/
